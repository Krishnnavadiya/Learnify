...
function AttachmentList({ link, createdby, usertype, isEnrolled }) {
    ...
    return (
        <>
            <li>
                <div className='download-pdf-btn' onClick={handleOpenPdfDialog}>
                    <IconButton
                    // onClick={handleOpenPdfDialog}
                    >
                        {
                            link?.split('/').pop().split('.').pop() === 'pdf' ?
                                <PictureAsPdfIcon style={{ fontSize: '25px', color: '#1976d2' }} />
                                : <VideoLibraryIcon style={{ fontSize: '25px', color: '#1976d2' }} />
                        }
...
export function CourseAccordion(props) {
    ...
    return (
        <div>
            <div className='section-unit-titles'>
                <div className='section-unit-items' onClick={toggleContent}>
                    <span className='unit-title'>{post.title}</span>
